...
public class TalkManager : MonoBehaviour
{
    ...
    void GenerateData()
    {
        ...
        talkData.Add(2000, new string[] { "여어.:1", "이 호수는 정말 아름답지?:0", "사실 이 호수에는 무언가 비밀이 숨겨져있다고 해.:1", "너무 궁금하지 않니?:2" });

        talkData.Add(3000, new string[] { "평범한 나무 상자다." });
        talkData.Add(4000, new string[] { "누군가 사용했던 흔적이 있는 책상이다." });

        // Quest Talk
        talkData.Add(10 + 1000, new string[] {"어서와.:0", "이 마을에 놀라운 전설이 있다는데:1", "오른쪽 호수 쪽에 루도가 알려줄거야!:0"});
        talkData.Add(11 + 1000, new string[] {"아직 못 만났어?:0", "루도는 오른쪽 호수에 있어.:0"});
        talkData.Add(11 + 2000, new string[] {"여어.:0", "이 호수의 전설을 들으러 온거야?:0", "그럼 일 좀 하나 해주면 좋을텐데...:1", "내 집 근처에 떨어진 동전 좀 주워줬으면 해!:1"});
